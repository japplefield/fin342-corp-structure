...
for sector in unique_sectors:
    ...
    plt.ylabel('Median Equity Change Attributable to Dividends / Normalized EBITDA')
    ax.text(0.5, -0.05, f'Median Quarterly Equity Change Attributable to Dividends / Normalized EBITDA for {sector}', ha="center", va="top", transform=ax.transAxes)
    ...
    plt.savefig(f'Equity_Bar_Charts/med_eq_divs_cng_ebitda_{sector.replace(" ", "_")}.png')
...
ax.set_ylabel('Median Equity Change Attributable to Dividends/ EBITDA')
ax.set_title('Median Equity Change Attributable to Dividends / EBITDA by Sector, Last 4 Quarters')
ax.set_xticks(x)
ax.set_xticklabels(meds.keys())
fig.autofmt_xdate()
ax.legend()
plt.savefig('Equity_Bar_Charts/med_eq_divs_cng_ebitda_all.png')
