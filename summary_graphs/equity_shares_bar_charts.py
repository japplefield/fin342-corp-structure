...
for sector in unique_sectors:
    ...
    plt.ylabel('Median Equity Change Attributable to Share Issuance (Repurchase) / Normalized EBITDA')
    ax.text(0.5, -0.05, f'Median Quarterly Equity Change Attributable to Share Issuance (Repurchase) / Normalized EBITDA for {sector}', ha="center", va="top", transform=ax.transAxes)
    ...
    plt.savefig(f'Equity_Bar_Charts/med_eq_shares_cng_ebitda_{sector.replace(" ", "_")}.png')
...
fig.autofmt_xdate()
ax.legend()
plt.savefig('Equity_Bar_Charts/med_eq_shares_cng_ebitda_all.png')
